...
class Parser:
    ...
    def parse(self):
        ...
        dates, auditors, pairs, pairs_time, professors = [], [], [], [], []
        ...
        for i in range(len(pairs_time)):
            ...
            for j in range(len(pairs_time[i])):
                pairs_objects.append(Pair(pairs[i][j], auditors[i][j], pairs_time[i][j], professors[i][j]))
...
class PairType:
    def __init__(self):
        self.practice = 'pr coming'
        self.laboratory = 'lb coming'
        self.lection = 'l coming'
